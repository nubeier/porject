...
<body>
		<div class="main">
			...
			<div class="search">
				...
				<?php	
					...
					//查询分类给的ID数量
					$BrandNum=mysql_num_rows($rstBrand);
					if($BrandNum==0){
	...
	//查询数据有几条进行分页
	$sql="select shop.*,brand.name bname,class.name cname from brand,class,shop where brand.class_id=class.id and shop.brand_id=brand.id and class_id={$class_id}";
		mysql_query("set names utf8");
		$rst=mysql_query($sql);
			...
			<!--floor 为商品-->
					...
					<div class="floorfooter">
						<div class="shop">
							<?php
							//进行分页
								$sqlShop="select shop.*,brand.name bname,class.name cname from brand,class,shop where shop.brand_id=brand.id and brand.class_id=class.id  and class_id={$_SESSION['class_id']} and shelf=1  limit ".($page-1)*$page_size.",$page_size";
								mysql_query("set names utf8");
			...
			<div class="page">
				...
				<ul>
					...
					<?php
						...
						if($page>1){
						echo "<a href=indexclass.php?page=1'>首页</a>";
						}
					...
					echo "<li>";
							...
							$return=$page-1;
							if($return>=2){
								echo "<a href='indexclass.php?page=$return'>上一页</a>";
							}
	...
	border:1px solid #888; color:red;'><b>$i</b></span>&nbsp;";
					}else{
						echo "<a href='indexclass.php?page=$i'>$i</a>&nbsp;"; 
					}
				}
					...
					echo "<li>";
						...
						$xia=$page+1;
				if($xia<=$page_num){
					echo "<a href='indexclass.php?page=$xia'>下一页</a>";
				}
				...
				if($page<$page_num){
					echo "<a href='indexclass.php?page=$page_num'>最后一页</a>";
				}
